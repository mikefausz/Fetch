...
public class Main {
    ...
    public static ArrayList<Request> selectUserOpenRequests(Connection conn, int userId, String open) throws SQLException {
        PreparedStatement stmt = conn.prepareStatement("SELECT users.name, requests.id, requests.request, requests.status FROM requests INNER JOIN users ON requests.user_id=users.id WHERE requests.status = ? AND requests.user_id = ?");
